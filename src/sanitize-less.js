      ...
      DOMPurify.addHook("uponSanitizeElement", (node, data) => {
        if (data && data.tagName === "a") {
          ...
          if (href) {
            ...
            // Check if the URL is within the same origin
            if (parsedHref.origin === window.location.origin) {
              // Keep the event handlers for onClick and onMouseover if the URL is within the same origin
              const allowedEvents = ['onclick', 'onmouseover'];
              for (let event of allowedEvents) {
                if (node.hasAttribute(event)) {
                  const eventHandler = node.getAttribute(event);
                  if (eventHandler.includes('javascript:')) {
                    node.removeAttribute(event);
                  }
                }
              }
            } else {
              // Remove the event handlers if the URL is not within the same origin
